...
from dataset import SIMPLE_CLEVRTEXdataset


FLAGS = flags.FLAGS
flags.DEFINE_bool('train', False, help='train from scratch')
flags.DEFINE_bool('resume', False, help='resume from ckpt')
...
# Logging & Sampling
flags.DEFINE_string('logdir', './logs/DDPM_SIMPLE_CLEVRTEX_EPS', help='log directory')
...
flags.DEFINE_integer('class_num', 10, help='number of class')
...
def train():
    ...
    # model setup
    #新增：传入图片类别总数FLAGS.class_num
    net_model = UNet(
        T=FLAGS.T, ch=FLAGS.ch, ch_mult=FLAGS.ch_mult, attn=FLAGS.attn,
        num_res_blocks=FLAGS.num_res_blocks, dropout=FLAGS.dropout,class_num=FLAGS.class_num)
    ...
    # log setup
        ...
        grid = (make_grid(next(iter(dataloader))[0][0][:FLAGS.sample_size])+1)/2
        #新增：保存数据集示例
        save_image(grid, os.path.join(FLAGS.logdir, 'example.png'))
    ...
    # start training
    with trange(current_step,FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            # train
            optim.zero_grad()
            ...
            #新增：传入图片类别信息
            obj_class=data[1].to(device)
            
            loss = trainer(x_0,obj_class).mean()
            loss.backward()
            ...
            if FLAGS.sample_step > 0 and step % FLAGS.sample_step == 0:
                ...
                with torch.no_grad():
                    for i in range(FLAGS.class_num):
                        x_0 = ema_sampler(x_T,torch.ones(FLAGS.sample_size, dtype=torch.long).to(device)*i)
                        grid = (make_grid(x_0)+1)/2
                        path = os.path.join(
                            FLAGS.logdir, 'sample', f'{step}_{i}.png')
                        save_image(grid, path)
                        writer.add_image('sample_%d'%(i), grid, step)
                net_model.train()
